public class Panel {
	...
	public int dd(int j, int k, int l) {
		return Surface.qg((jg * j) / 114, (kg * k) / 114, (lg * l) / 176);
	}
	...
	public void wd(int j, int k, int l, int i1) {
		...
		if (l == 1) {
			for (int j1 = 0; j1 < ue; j1++) {
				if (we[j1] && hf[j1] == 10 && of >= ff[j1] && pf >= gf[j1] && of <= ff[j1] + _fldif[j1]
						&& pf <= gf[j1] + jf[j1])
					ze[j1] = true;
				if (we[j1] && hf[j1] == 14 && of >= ff[j1] && pf >= gf[j1] && of <= ff[j1] + _fldif[j1]
						&& pf <= gf[j1] + jf[j1])
					cf[j1] = 1 - cf[j1];
			}
		...
		if (l == 1 || tf > 20) {
			for (int k1 = 0; k1 < ue; k1++)
				if (we[k1] && hf[k1] == 15 && of >= ff[k1] && pf >= gf[k1] && of <= ff[k1] + _fldif[k1]
						&& pf <= gf[k1] + jf[k1])
					ze[k1] = true;
		...
		for (int j = 0; j < ue; j++)
			if (we[j])
				if (hf[j] == 0)
					...
					nd(j, ff[j] - surface.kf(mf[j], lf[j]) / 2, gf[j], mf[j], lf[j]);
				else if (hf[j] == 2)
	...
	protected void id(int j, int k, int l, int i1, int j1) {
		surface.eg(k, l, i1, j1, 0xffffff);
		surface.xg(k, l, i1, cg);
		surface.ch(k, l, j1, cg);
		surface.xg(k, (l + j1) - 1, i1, fg);
		surface.ch((k + i1) - 1, l, j1, fg);
		if (cf[j] == 1) {
			for (int k1 = 0; k1 < j1; k1++) {
				surface.xg(k + k1, l + k1, 1, 0);
				surface.xg((k + i1) - 1 - k1, l + k1, 1, 0);
			}
		}
	}

	protected void nd(int j, int k, int l, String s, int i1) {
		int j1 = l + surface.tg(i1) / 3;
		pd(j, k, j1, s, i1);
	}
	...
	protected void yc(int j, int k, int l, int i1, int j1, String s, int k1) {
		...
		if (sf == j)
			s = s + "*";
		int i2 = l + surface.tg(k1) / 3;
		pd(j, k, i2, s, k1);
	}

	public void od(int j, int k, int l, int i1) {
		surface.yf(j, k, j + l, k + i1);
		surface.mg(j, k, l, i1, fg, cg);
		if (hg) {
			for (int j1 = j - (k & 0x3f); j1 < j + l; j1 += 128) {
				for (int k1 = k - (k & 0x1f); k1 < k + i1; k1 += 128)
					surface.wg(j1, k1, 6 + ig, 128);
			}
		}
		surface.xg(j, k, l, cg);
		surface.xg(j + 1, k + 1, l - 2, cg);
		surface.xg(j + 2, k + 2, l - 4, dg);
		surface.ch(j, k, i1, cg);
		surface.ch(j + 1, k + 1, i1 - 2, cg);
		surface.ch(j + 2, k + 2, i1 - 4, dg);
		surface.xg(j, (k + i1) - 1, l, fg);
		surface.xg(j + 1, (k + i1) - 2, l - 2, fg);
		surface.xg(j + 2, (k + i1) - 3, l - 4, eg);
		surface.ch((j + l) - 1, k, i1, fg);
		surface.ch((j + l) - 2, k + 1, i1 - 2, fg);
		surface.ch((j + l) - 3, k + 2, i1 - 4, eg);
		surface.zf();
	}

	public void md(int j, int k, int l, int i1) {
		surface.eg(j, k, l, i1, 0);
		surface.xf(j, k, l, i1, zf);
		surface.xf(j + 1, k + 1, l - 2, i1 - 2, ag);
		surface.xf(j + 2, k + 2, l - 4, i1 - 4, bg);
		surface.eh(j, k, 2 + ig);
		surface.eh((j + l) - 7, k, 3 + ig);
		surface.eh(j, (k + i1) - 7, 4 + ig);
		surface.eh((j + l) - 7, (k + i1) - 7, 5 + ig);
	}
	...
	protected void jd(int j, int k, int l, int i1, int j1, int k1, String as[], int l1, int i2) {
		int j2 = j1 / surface.tg(k1);
		if (i2 > l1 - j2)
			i2 = l1 - j2;
		if (i2 < 0)
			i2 = 0;
		...
		int l2 = j1 - j2 * surface.tg(k1);
		int j3 = l + (surface.tg(k1) * 5) / 6 + l2 / 2;
	...
	protected void sd(int j, int k, int l, int i1, int j1, int k1) {
		int l1 = (j + l) - 12;
		surface.xf(l1, k, 12, i1, 0);
		surface.eh(l1 + 1, k + 1, ig);
		surface.eh(l1 + 1, (k + i1) - 12, 1 + ig);
		surface.xg(l1, k + 13, 12, 0);
		surface.xg(l1, (k + i1) - 13, 12, 0);
		surface.mg(l1 + 1, k + 14, 11, i1 - 27, uf, vf);
		surface.eg(l1 + 3, j1 + k + 14, 7, k1, xf);
		surface.ch(l1 + 2, j1 + k + 14, k1, wf);
		surface.ch(l1 + 2 + 8, j1 + k + 14, k1, yf);
	}
	...
	protected void vc(int j, int k, int l, int i1, String as[]) {
		...
		for (int l1 = 0; l1 < k1; l1++) {
			j1 += surface.kf(as[l1], i1);
			if (l1 < k1 - 1)
				j1 += surface.kf("  ", i1);
		}

		int i2 = k - j1 / 2;
		int j2 = l + surface.tg(i1) / 3;
		for (int k2 = 0; k2 < k1; k2++) {
			...
			if (ef[j])
				...
				l2 = 0;
			if (of >= i2 && of <= i2 + surface.kf(as[k2], i1) && pf <= j2 && pf > j2 - surface.tg(i1)) {
				if (ef[j])
					l2 = 0x808080;
				else
					l2 = 0xffffff;
			...
			if (cf[j] == k2)
				if (ef[j])
					...
					l2 = 0xc00000;
			surface.tf(as[k2], i2, j2, i1, l2);
			i2 += surface.kf(as[k2] + "  ", i1);
		}
	...
	protected void ic(int j, int k, int l, int i1, String as[]) {
		...
		for (int l1 = 0; l1 < j1; l1++) {
			...
			if (ef[j])
				...
				i2 = 0;
			int j2 = surface.kf(as[l1], i1);
			if (of >= k - j2 / 2 && of <= k + j2 / 2 && pf - 2 <= k1 && pf - 2 > k1 - surface.tg(i1)) {
				if (ef[j])
					i2 = 0x808080;
				else
					i2 = 0xffffff;
			...
			if (cf[j] == l1)
				if (ef[j])
					...
					i2 = 0xc00000;
			surface.tf(as[l1], k - j2 / 2, k1, i1, i2);
			k1 += surface.tg(i1);
		}

	}

	protected void mc(int j, int k, int l, int i1, int j1, int k1, String as[], int l1, int i2) {
		int j2 = j1 / surface.tg(k1);
		if (j2 < l1) {
			int k2 = (k + i1) - 12;
			int i3 = ((j1 - 27) * j2) / l1;
		...
		for (int l3 = i2; l3 < l1; l3++) {
			...
			if (ef[j])
				...
				j4 = 0;
			if (of >= k + 2 && of <= k + 2 + surface.kf(as[l3], k1) && pf - 2 <= j3
					&& pf - 2 > j3 - surface.tg(k1)) {
				if (ef[j])
					j4 = 0x808080;
				else
					j4 = 0xffffff;
	...
	public int lc(int j, int k, int l, int i1, int j1, int k1, boolean flag, boolean flag1) {
		hf[ue] = 5;
		we[ue] = true;
		ye[ue] = flag;
		ze[ue] = false;
	...
	public int fd(int j, int k, int l, int i1, int j1, int k1, boolean flag, boolean flag1) {
		hf[ue] = 6;
		we[ue] = true;
		ye[ue] = flag;
		ze[ue] = false;
