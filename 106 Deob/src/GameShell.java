...
public class GameShell extends Applet implements Runnable {
	...
	public final void hj(int j, int l, String s, boolean flag) {
		applet_started = false;
		System.out.println("Started application");
		np = j;
		op = l;
		reference = new GameFrame(this, j, l, s, flag, false);
		zp = 1;
		pp = new Thread(this);
		pp.start();
		pp.setPriority(1);
	...
	public final boolean applet_started() {
		return applet_started;
	}
	...
	public final void tj(int j, int l, String s, boolean flag) {
		if (reference != null) {
			...
			np = j;
			op = l;
			reference = new GameFrame(this, j, l, s, flag, applet_started);
			return;
		}
	}
	...
	public final Image createImage(int j, int l) {
		if (reference == null)
			...
			return reference.createImage(j, l);
	}
	...
	public final void run() {
		...
		while (vp >= 0) {
			...
			xj();
			dr = (1000 * i1) / (qp * 256);
			if (applet_started && j == 0)
				showStatus("Fps:" + dr + "Del:" + j1);
			if (reference != null && (reference.c() != np || reference.d() != op))
				ak(reference.c(), reference.d());
		}
		if (vp == -1)
			nj();
		pp = null;
	...
	public void pj() {
		try {
			byte abyte0[] = Data.pn("jagex.jag");
			byte abyte1[] = Data.ln("logo.tga", 0, abyte0);
			hq = oj(abyte1);
			Surface.lf(Data.ln("h11p.jf", 0, abyte0));
			Surface.lf(Data.ln("h12b.jf", 0, abyte0));
			Surface.lf(Data.ln("h12p.jf", 0, abyte0));
			Surface.lf(Data.ln("h13b.jf", 0, abyte0));
			Surface.lf(Data.ln("h14b.jf", 0, abyte0));
			Surface.lf(Data.ln("h16b.jf", 0, abyte0));
			Surface.lf(Data.ln("h20b.jf", 0, abyte0));
			Surface.lf(Data.ln("h24b.jf", 0, abyte0));
			return;
	...
	public void draw_loading_progress(int j, String s) {
		int l = (np - 281) / 2;
		int i1 = (op - 148) / 2;
		l += 2;
		i1 += 90;
	...
	public byte[] unpack_archive(String s, String s1, int j) throws IOException {
		int l = 0;
		int i1 = 0;
		int j1 = 0;
		byte abyte0[] = null;
		while (l < 2)
			try {
				draw_loading_progress(j, "Loading " + s1 + " - 0%");
				if (l == 1)
					s = s.toUpperCase();
				java.io.InputStream inputstream = Data.open_stream(s);
				...
				draw_loading_progress(j, "Loading " + s1 + " - 5%");
				int k1 = 0;
				abyte0 = new byte[j1];
				while (k1 < j1) {
					int l1 = j1 - k1;
					...
					k1 += l1;
					draw_loading_progress(j, "Loading " + s1 + " - " + (5 + (k1 * 95) / j1) + "%");
				}
				l = 2;
		...
		draw_loading_progress(j, "Unpacking " + s1);
		if (j1 != i1) {
			byte abyte1[] = new byte[i1];
			BZipDecompressor.ik(abyte1, i1, abyte0, j1, 0);
			return abyte1;
		} else {
			return abyte0;
		}
	...
	private int rp;
	private long sp[];
	static GameFrame reference = null;
	private boolean applet_started;
	private int vp;
	private int wp;
	public int xp;
	public int yp;
