...
public class Surface implements ImageProducer, ImageObserver {
	public Surface(int j, int k, int l, Component component) {
		rk = true;
		wk = false;
		al = false;
		fk = component;
	...
	public synchronized void bg() {
		if (consumer == null) {
			...
			consumer.setPixels(0, 0, xj, yj, ck, dk, 0, xj);
	...
	public void mf() {
		...
		for (int k = 0; k < l; k++) {
			int j = dk[k] & 0xffffff;
			dk[k] = (j >>> 1 & 0x7f7f7f) + (j >>> 2 & 0x3f3f3f) + (j >>> 3 & 0x1f1f1f)
					+ (j >>> 4 & 0xf0f0f);
		}
	...
	public void ng(byte abyte0[], int j, int k, boolean flag, int l, int i1, boolean flag1) {
		...
		for (int i2 = 0; i2 < 256; i2++) {
			ai[i2] = 0xff000000 + ((abyte0[j + 20 + i2 * 3] & 0xff) << 16)
					+ ((abyte0[j + 19 + i2 * 3] & 0xff) << 8) + (abyte0[j + 18 + i2 * 3] & 0xff);
			if (ai[i2] == -65281)
				l1 = i2;
		}
	...
	private void nf(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
		int j2 = -(i1 >> 2);
		i1 = -(i1 & 3);
	...
	private void ig(int ai[], byte abyte0[], int ai1[], int j, int k, int l, int i1, int j1, int k1,
			int l1) {
		int i2 = -(l >> 2);
		l = -(l & 3);
	...
	private void wf(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
	...
	private void lg(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
			int j2) {
		int k2 = 256 - j2;
		for (int l2 = -j1; l2 < 0; l2 += i2) {
			for (int i3 = -i1; i3 < 0; i3++) {
				...
				if (j != 0) {
					int j3 = ai[l];
					ai[l++] = ((j & 0xff00ff) * j2 + (j3 & 0xff00ff) * k2 & 0xff00ff00)
							+ ((j & 0xff00) * j2 + (j3 & 0xff00) * k2 & 0xff0000) >> 8;
				} else {
					l++;
				}
			}
	...
	private void ih(int ai[], byte abyte0[], int ai1[], int j, int k, int l, int i1, int j1, int k1,
			int l1, int i2) {
		int j2 = 256 - i2;
		for (int k2 = -i1; k2 < 0; k2 += l1) {
			for (int l2 = -l; l2 < 0; l2++) {
				...
				if (i3 != 0) {
					i3 = ai1[i3 & 0xff];
					int j3 = ai[k];
					ai[k++] = ((i3 & 0xff00ff) * i2 + (j3 & 0xff00ff) * j2 & 0xff00ff00)
							+ ((i3 & 0xff00) * i2 + (j3 & 0xff00) * j2 & 0xff0000) >> 8;
				} else {
					k++;
				}
			}
	...
	private void dg(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
		...
		try {
			...
			for (int l3 = -l1; l3 < 0; l3 += l2) {
				...
				for (int j4 = -k1; j4 < 0; j4++) {
					...
					if (j != 0) {
						int k4 = ai[i1];
						ai[i1++] = ((j & 0xff00ff) * i3 + (k4 & 0xff00ff) * j3 & 0xff00ff00)
								+ ((j & 0xff00) * i3 + (k4 & 0xff00) * j3 & 0xff0000) >> 8;
					} else {
						i1++;
					}
					k += i2;
	...
	private void kh(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
			int j2, int k2, int l2, int i3) {
		int j3 = i3 >> 16 & 0xff;
		int k3 = i3 >> 8 & 0xff;
		int l3 = i3 & 0xff;
	...
	private void gg(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
		for (j = l1; j < 0; j++) {
	...
	private void kg(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
	...
	public void cg(int j, int k, int l, int i1, int j1, int k1, int l1, int i2, boolean flag) {
		try {
			if (k1 == 0)
				k1 = 0xffffff;
			if (l1 == 0)
			...
			if (l1 == 0xffffff) {
				...
				if (!flag) {
					...
					pf(dk, ik[j1], jk[j1], wj, (kk[j1] << 16) - l2 - 1, i3, k4, l, i1, -k3, l3, j2, k1, j3,
							i4, j5);
					return;
				}
			}
			if (hk[j1] != null)
				if (!flag) {
					...
					hg(dk, hk[j1], wj, (kk[j1] << 16) - l2 - 1, i3, k4, l, i1, -k3, l3, j2, k1, l1, j3, i4,
							j5);
					return;
				}
			if (!flag) {
				...
				hh(dk, ik[j1], jk[j1], wj, (kk[j1] << 16) - l2 - 1, i3, k4, l, i1, -k3, l3, j2, k1, l1, j3,
						i4, j5);
				return;
			}
		} catch (Exception _ex) {
	...
	private void jh(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
	...
	private void hg(int ai[], int ai1[], int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
	...
	private void pf(int ai[], byte abyte0[], int ai1[], int j, int k, int l, int i1, int j1, int k1,
	...
	private void hh(int ai[], byte abyte0[], int ai1[], int j, int k, int l, int i1, int j1, int k1,
	...
	private void jf(int ai[], byte abyte0[], int j, int k, int l, int i1, int j1, int k1, int l1) {
		int i2 = -(i1 >> 2);
		i1 = -(i1 & 3);
