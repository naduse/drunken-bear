...
public class Data {
	...
	public static void read_fully(String s, byte abyte0[], int i) throws IOException {
		InputStream inputstream = open_stream(s);
	...
	public static void p32(byte buffer[], int position, int val) {
	...
	public static void p64(byte buffer[], int position, long val) {
	...
	public static int gu16(byte buffer[], int position) {
		return ((buffer[position] & 0xff) << 8) + (buffer[position + 1] & 0xff);
	}

	public static int gu32(byte abyte0[], int i) {
		return ((abyte0[i] & 0xff) << 24) + ((abyte0[i + 1] & 0xff) << 16)
				+ ((abyte0[i + 2] & 0xff) << 8) + (abyte0[i + 3] & 0xff);
	}

	public static long gu64(byte abyte0[], int i) {
		return (((long) gu32(abyte0, i) & 0xffffffffL) << 32)
				+ ((long) gu32(abyte0, i + 4) & 0xffffffffL);
	}

	public static int g16(byte buffer[], int position) {
		int j = unsign(buffer[position]) * 256 + unsign(buffer[position + 1]);
		if (j > 32767)
			j -= 0x10000;
		return j;
	}

	public static int gbits(byte buffer[], int i, int j) {
		int k = i >> 3;
		int l = 8 - (i & 7);
		int i1 = 0;
		for (; j > l; l = 8) {
			i1 += (buffer[k++] & bit_mask[l]) << j - l;
			j -= l;
		}

		if (j == l)
			i1 += buffer[k] & bit_mask[l];
		else
			i1 += buffer[k] >> l - j & bit_mask[j];
		return i1;
	}

	public static String format_string(String s, int i) {
		String s1 = "";
		for (int j = 0; j < i; j++)
			if (j >= s.length()) {
				s1 = s1 + " ";
	...
	public static long encode_47(String s) {
		s = s.trim();
		s = s.toLowerCase();
		long l = 0L;
		int i = 0;
	...
	public static byte[] pn(String s) throws IOException {
		...
		if (k != j) {
			...
			BZipDecompressor.ik(abyte1, j, abyte0, k, 0);
			return abyte1;
		} else {
			return abyte0;
		}
	...
	public static byte[] eo(String s, int i, byte abyte0[], byte abyte1[]) {
		...
		for (int j1 = 0; j1 < j; j1++) {
			...
			if (k1 == k) {
				if (l1 != i2) {
					BZipDecompressor.ik(abyte1, l1, abyte0, i2, i1);
				} else {
			...
			for (int j = 0; j < s.length(); j++) {
				...
				if (c >= 'a' && c <= 'z' || c >= '0' && c <= '9' || c == '\'') {
					...
					for (int i1 = 0; i1 < s1.length(); i1++)
						wfb[i++] = (byte) get_keycode(s1.charAt(i1));
	...
	private static int get_keycode(char c) {
		if (c >= 'a' && c <= 'z')
			return c - 97;
		if (c >= '0' && c <= '9')
			return (c + 26) - 48;
	...
	private static boolean tn(char c, char c1, int i) {
		...
		if (c == 'k' && c1 == 'c')
			return true;
		return i >= 5 && (c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u' || c == 'y')
				&& (c1 == 'a' || c1 == 'e' || c1 == 'i' || c1 == 'o' || c1 == 'u' || c1 == 'y');
	}
	...
	public static String tfb[] = { "bum" };
	public static int ufb = 1;
	public static String vfb[] = { "hello" };
	public static byte wfb[] = new byte[200];
	static char xfb[] = new char[1000];
	private static char valid_chars[] = { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L',
